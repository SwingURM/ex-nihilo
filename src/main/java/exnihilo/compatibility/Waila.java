...
public class Waila implements IWailaDataProvider {
    ...
    public String getBarrelDisplay(TileEntityBarrel.BarrelMode mode, TileEntityBarrel barrel) {
        ...
        switch (mode) {
            ...
            case FLUID -> {
                if (barrel.isFull()) return barrel.fluid.getFluid()
                    .getName();
                return barrel.fluid.getFluid()
                    .getName() + " " + format.format((barrel.getVolume() * 100.0F)) + "%";
            }
            case COMPOST -> {
